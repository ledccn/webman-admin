...
class TableController extends Base
{
    ...
    public function show(Request $request): Response
    {
        ...
        $total = Util::db()->select("SELECT count(*)total FROM sqlite_master WHERE type=\"table\"")[0]->total ?? 0;
        $tables = Util::db()->select("SELECT name as TABLE_NAME, tbl_name as TABLE_COMMENT FROM  sqlite_master WHERE type=\"table\" order by $field $order limit $offset,$limit");
        ...
        if ($tables) {
            ...
            foreach ($tables as $table) {
                $table->TABLE_ROWS = $table_rows_count[$table->TABLE_NAME] ?? 0;
            }
        }
    ...
    protected function createModel($class, $namespace, $file, $table)
    {
        ...
        try {
            //plugin.admin.sqlite
            foreach (Util::tableInfo($table) as $item) {
                //sqlite3
                $field = $item->name;
                $type = $item->type;
                $COLUMN_COMMENT = $item->name;
                $type = Util::getTypeBySqlite($type);
                if ($field === 'id') {
                    $pk = $field;
                    $COLUMN_COMMENT .= "(主键)";
...
;
                ...
                $properties .= " * @property $type \${$field} {$COLUMN_COMMENT}\n";
                $columns[$field] = $field;
            }
...
EOF;
     ...
     * @return mixed
     */
    protected function createColumn($column, Blueprint $table): mixed
    {
        $method = $column['type'];
        $args = [$column['field']];
